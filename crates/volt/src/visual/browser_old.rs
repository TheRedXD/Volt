...
use std::{
    borrow::Cow, collections::HashMap, f32::consts::FRAC_PI_2, fs::read_dir, iter::Iterator, ops::BitOr, path::{Path, PathBuf}, rc::Rc, string::ToString, sync::{Arc, RwLock}, task::Poll, thread::spawn, time::{Duration, Instant}, vec::{self, IntoIter}
};
use strum::Display;
use tap::Pipe;
...
impl Browser {
    ...
    fn add_files(&mut self, ui: &mut Ui, scroll_area: ScrollArea, browser_width: f32) -> Response {
        ...
        if !self.flattened_cache_valid {
            let open_paths = self.open_paths.clone();
            let new_entries = open_paths.iter().fold(Vec::new(), |mut entries, path| {
                let cached_entries_ref = self.cached_entries.clone();
                let cached_entry_kinds_ref = &self.cached_entry_kinds.clone();
                let expanded_paths_ref = &self.expanded_paths.clone();
                let entries_ref = &mut entries;
                self.entries(entries_ref, path, 0, cached_entries_ref, cached_entry_kinds_ref, expanded_paths_ref);
                entries
            });
    
            let has_pending = new_entries.iter().any(|entry| matches!(entry.data, Poll::Pending));
            if !has_pending && !new_entries.is_empty() {
                self.flattened_entries_cache = Arc::new(new_entries);
                self.flattened_cache_valid = true;
            } else {
                self.flattened_entries_cache = Arc::new(new_entries);
            }
        }
        let entries = self.flattened_entries_cache.clone();
        scroll_area
            .show_rows(ui, Self::ENTRY_HEIGHT, self.flattened_entries_cache.len(), |ui, row_range| {
                egui::Frame::default()
                    ...
                    .show(ui, |ui| {
                        ui.vertical(|ui| {
                            ...
                            for entry in entries.iter().skip(row_range.start).take(row_range.len() + 8) {
    ...
    fn list_cached<'a>(path: &Path, cached_entries: Arc<RwLock<FsWatcherCache<CachedEntries>>>, cached_entry_kinds: Arc<RwLock<FsWatcherCache<EntryKind>>>) -> &'a mut CachedEntries {
        for event in cached_entries.write().unwrap().rx.try_iter() {
            let event = event.unwrap();
            match event.kind {
                EventKind::Access(_) => {}
                _ => {
                    for path in event.paths.iter().map(|path| if path.is_dir() { path } else { path.parent().unwrap() }) {
                        ...
                        cached_entries.write().unwrap().data.remove(path);
                    }
                }
            }
        }
        let cached_entries_read = cached_entries.read().unwrap();

        cached_entries.write().unwrap().data.entry(path.to_path_buf()).or_insert_with(|| {
            trace!("list cache miss for {:?}", path);
            let watch_result = cached_entries.write().unwrap().watcher.watch(path.parent().unwrap_or(path), RecursiveMode::NonRecursive);
            if let Err(error) = watch_result {
                error!("Unexpected error while trying to watch directory: {:?}", error);
            }
            let (tx, rx) = bounded(1);
            ...
            let cached_entry_kinds = cached_entry_kinds.clone();
            spawn(move || {
    ...
    fn entries(
        ...
        path: &Path,
        mut depth: usize,
        cached_entries: &Arc<&mut FsWatcherCache<CachedEntries>>,
        cached_entry_kinds: &Arc<RwLock<FsWatcherCache<EntryKind>>>,
        ...
        depth += 1;
        let CachedEntries { data, rx } = Self::list_cached(path, cached_entries.clone(), cached_entry_kinds);
        match data {
            Poll::Ready(list) => {
                for (kind, entry) in list.clone() {
                    ...
                    if expanded_paths.iter().any(|expanded| **expanded == *entry) {
                        Self::entries(self, entries, &entry, depth, cached_entries, cached_entry_kinds, expanded_paths);
                    }
            ...
            Poll::Pending => match rx.try_recv() {
                ...
                Err(TryRecvError::Empty) => {
                    self.flattened_cache_valid = false;
        ...
        let mut add_contents = |ui: &mut Ui| {
            ui.horizontal(|ui| {
                ui.add(Image::new(include_image!("../images/icons/file_audio.svg"))).union(ui.add(button(theme))).pipe(|response| {
                    ...
                    // let is_playing = preview.is_playing();
                    if let Some(current_data) = preview.data()
                        && current_data.path.as_ref().is_some_and(|current_path| **current_path == path)
                    {
                        // if is_playing && (self.preview_last_draw.elapsed().as_millis() >= 500) {
                        //     ui.ctx().request_repaint_after(Duration::from_millis(500));
                        //     self.preview_last_draw = Instant::now();
                        // }
                        response
                            | ui.label(format!(
                                "{:>02}:{:>02} of {:>02}:{:>02}",
...
impl Widget for &mut Browser {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        ui.vertical(|ui| {
            ...
            ui.add_space(4.);
            ui.visuals_mut().extreme_bg_color = self.theme.browser_scrollbar;
